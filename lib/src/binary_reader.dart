...
///
/// Every read is bounds-checked against what is left, and a read that runs
/// past the end throws [FormatException]. The lengths in the file are just
/// numbers from an untrusted source: they size allocations and drive the
/// parse, so a corrupt or truncated model has to fail as the documented
/// exception rather than as a `RangeError` from somewhere deep inside.
class BinaryReader {
  ...
  /// Fails unless [count] more bytes are available.
  void _require(int count) {
    if (count < 0 || count > remaining) {
      throw FormatException(
        'the model is truncated or corrupt: $count bytes wanted at offset '
        '$_offset, $remaining left',
      );
    }
  }
  ...
  Uint8List cString() {
    final start = _offset;
    while (_offset < bytes.length && bytes[_offset] != 0) {
      _offset++;
    }

    if (_offset == bytes.length) {
      throw FormatException(
        'the model is truncated or corrupt: unterminated string at offset '
        '$start',
      );
    }
